...
SESSIONS_TABLE=${SESSIONS_TABLE:-llmxt-sessions}
...
echo "Creating $SESSIONS_TABLE table..."
aws dynamodb create-table \
  ...
  || echo "Table $SESSIONS_TABLE may already exist"
...
aws dynamodb describe-table --table-name "$SESSIONS_TABLE" --region "$REGION" --query 'Table.[TableName,TableStatus,ItemCount]' --output table

echo ""
echo "Next steps:"
echo "1. Update your .env file with these table names"
echo "2. Ensure AWS credentials are configured (IAM role or credentials)"
echo "3. Configure Dodo webhook URL: https://your-domain.com/api/webhook"
